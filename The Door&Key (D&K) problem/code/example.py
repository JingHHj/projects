...
def example_use_of_gym_env():
    ...
    # env, info = load_env("./envs/doorkey-8x8-shortcut.env")  # original
    env, info = load_env("./envs/known_envs/example-8x8.env")  # modified
    ...
    agent_dir = env.unwrapped.dir_vec  # or env.agent_dir

    # Get the cell in front of the agent
    front_cell = env.unwrapped.front_pos  # == agent_pos + agent_dir

    # Access the cell at coord: (2,3)
    cell = env.unwrapped.grid.get(2, 3)  # NoneType, Wall, Key, Goal

    # Get the door status
    door = env.unwrapped.grid.get(info["door_pos"][0], info["door_pos"][1])
    is_open = door.is_open
    is_locked = door.is_locked

    # Determine whether agent is carrying a key
    is_carrying = env.unwrapped.carrying is not None
    ...
    print("Step Count: {}".format(env.unwrapped.step_count))
